...
describe('Contracts', () => {
  it('Messages do not sort', async function() {
    ...
    const tx = await local.transaction(['currency', 'datxos.token'], ({currency, eosio_token}) => {
      // make sure {account: 'datxos.token', ..} remains first
    ...
    assert.equal(tx.transaction.transaction.actions[0].account, 'datxos.token')
...
describe('keyProvider', () => {
  ...
  it('per-action', async function() {
    ...
    const token = await eos.contract('datxos.token')
...
describe('transactions', () => {
  ...
  it('create asset', async function() {
    ...
    const auth = {authorization: 'datxos.token'}
    await eos.create('datxos.token', '10000 ' + randomAsset(), auth)
    await eos.create('datxos.token', '10000.00 ' + randomAsset(), auth)
  })
  ...
  it('action to contract', () => {
    return Eos({signProvider}).contract('datxos.token').then(token => {
  ...
  it('action to contract atomic', async function() {
    ...
    await assertTr(await eos.transaction(['datxos.token'], ({eosio_token}) => trTest(eosio_token)))
    await assertTr(await eos.transaction('datxos.token', eosio_token => trTest(eosio_token)))
  })
  ...
  it('action to contract (contract tr nesting)', function () {
    ...
    return tn.contract('datxos.token').then(eosio_token => {
